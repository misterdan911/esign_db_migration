...
$resTblSignature = $dbProSibela->query($qTblSignature);

echo "Migrasi tabel promise_siplang.tbl_signature..." . PHP_EOL;

while ($objTblSignature = $dbProSibela->fetch_object($resTblSignature))
{
    ...
    $resTblSignatureOtp = $dbProSibela->query($sTblSignatureOtp);
    $objTblSignatureOtp = $dbProSibela->fetch_object($resTblSignatureOtp);

    if (!empty($objTblSignatureOtp)) {
        $otp = $objTblSignatureOtp->otp;
        $expired_time = prepareString($dbProSibela, $objTblSignatureOtp->time_expired);
        $udcr = prepareString($dbProSibela, $objTblSignatureOtp->time);
